...
@section('content')
    ...
    <div class="row" id="quiz-content">
        ...
        <div class="col-md-9 quiz-{{ $quiz->id }}">
            ...
            <form action="{{ route('module.quiz_mobile.doquiz.submit', ['quiz_id' => $quiz->id, 'part_id' => $part->id, 'attempt_id' => $attempt->id]) }}" method="post" class="form-ajax text-center" id="form-submit" data-success="submit_success">
                <div class="card">
                    <div class="card-header">
                    ...
                    <div class="card-body">
                        ...
                        @if($attempt_finish)
                            <a href="{{ route('module.quiz_mobile.doquiz.index', [ 'quiz_id' => $quiz->id, 'part_id' => $part->id]) }}" class="btn btn-info"><i class="fa fa-mail-reply"></i> Trở về màn hình kỳ thi</a>
                        @endif
                        @if(!$attempt_finish)
                        <button type="submit" class="btn btn-primary send-quiz"><i class="fa fa-send-o"></i> Nộp bài thi</button>
                        @endif
                    </div>
                </div>
            </form>
    ...
    <template id="question-template">
        <div class="question-item" id="q{qid}" data-qid="{qid}">
            ...
            <div class="row">
                <div class="col-md-2">
                    <div class="info">
                        <h6 class="no">{{ trans('backend.question') }} <span class="qno">{index}</span></h6>
...
@section('footer')
    <script type="text/javascript">
        ...
        var quiz_id = '{{ $quiz->id }}';
        var quiz_url = '{{ route('module.quiz_mobile.doquiz.do_quiz', [
            'quiz_id' => $quiz->id,
            'part_id' => $part->id,
