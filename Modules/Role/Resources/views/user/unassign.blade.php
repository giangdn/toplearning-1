...
@section('breadcrumb')
    <div class="ibox-content forum-container">
        <h2 class="st_title"><i class="uil uil-apps"></i>
            ...
            <a href="{{ route('backend.roles.user.assign_role', ['role' => $role->id]) }}">{{ trans('backend.role') }} {{ $role->name }}</a>
            <i class="uil uil-angle-right"></i>
            <span class="font-weight-bold">{{ trans('backend.list_employee') }}</span>
        </h2>
    </div>
@endsection
@section('content')
        ...
        <div class="row">
            ...
            <div class="col-md-12 text-right act-btns">
                <div class="pull-right">
                    <div class="btn-group">
                        <button type="submit" id="btnSave" class="btn btn-primary"><i class="fa fa-plus-circle"></i> {{ trans('backend.save') }}</button>
                    </div>
                </div>
            </div>
        </div>
