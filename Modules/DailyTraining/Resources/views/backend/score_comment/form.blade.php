...
@section('breadcrumb')
    <div class="ibox-content forum-container">
        <h2 class="st_title"><i class="uil uil-apps"></i>
            ...
            <a href="{{ route('module.daily_training.score_comment') }}">{{ trans('backend.setting_comment') }}</a>
...
<div role="main">
    <form method="post" action="{{ route('module.daily_training.score_comment.save') }}" class="form-validate form-ajax" role="form" enctype="multipart/form-data">
        ...
        <div class="row">
            ...
            <div class="col-md-4 text-right">
                <div class="btn-group act-btns">
                    @canany(['score-comment-create','score-comment-edit'])
                        <button type="submit" class="btn btn-primary" data-must-checked="false"><i class="fa fa-save"></i> &nbsp; {{ trans('backend.save') }}</button>
                    @endcan
                    <a href="{{ route('module.daily_training.score_comment') }}" class="btn btn-warning"><i class="fa fa-times-circle"></i> {{ trans('backend.cancel') }}</a>
                </div>
            </div>
        </div>
        <div class="clear"></div>
