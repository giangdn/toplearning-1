    ...
    <div class="row">
        <div class="col-md-9">
            <div class="form-group row">
                ...
                <div class="col-md-9">
                    <select name="quiz_template_id" id="quiz_template_id" class="form-control select2" data-placeholder="-- Chọn bộ đề --" {{ $result ? 'disabled' : '' }}>
                        <option value=""></option>
            ...
            {{--<div class="form-group row">
                <div class="col-sm-3 control-label">
                    <label for="unit_id">{{ trans('backend.unit') }} @if($is_unit > 0)<span class="text-danger">*</span> @endif</label>
                </div>
                <div class="col-md-6">
                    <div class="w-75">
                        <select name="unit_id" id="unit_id" class="form-control load-unit-quiz" data-placeholder="-- {{ trans('backend.choose_unit') }} --" @if($is_unit > 0) disabled @endif>
                            @if($is_unit > 0)
                                <option value="{{ $unit_user->id }}" selected>{{ $unit_user->code }} - {{ $unit_user->name}}</option>
                            @endif
                            @if($unit->id)
        ...
        <div class="col-md-3">
            <div class="form-group text-right">
                <div class="btn-group act-btns">
                    @if(!$result || $model->quiz_type != 1)
                        ...
                        @endcanany
                    @endif
                        <a href="{{ route('module.quiz.manager') }}" class="btn btn-warning"><i class="fa fa-times-circle"></i> {{ trans('backend.cancel') }}</a>
                </div>
            </div>
            <p></p>
            <div class="form-group">
                <label class="w-100">{{trans('backend.picture')}} (300 x 160) <a href="javascript:void(0)" id="select-image" class="float-right">{{trans('backend.choose_picture')}}</a></label>
            ...
            <div class="form-group">
                ...
                <select name="quiz_type" id="quiz_type" class="form-control select2" data-placeholder="{{trans('backend.choose_form')}}" required>
                    <option value=""></option>
                    @if($model->quiz_type == 1)
                        <option value="1" {{ $model->quiz_type == 1 ? 'selected' : '' }}> {{trans('backend.online')}}</option>
                    @endif
                    @if($is_unit != 1)
                    {{-- <option value="1" {{ $model->quiz_type == 1 ? 'selected' : '' }}> {{trans('backend.online')}}</option> --}}
                    <option value="2" {{ $model->quiz_type == 2 ? 'selected' : '' }}> {{trans('backend.offline')}}</option>
                    @endif
            ...
            <div class="form-group">
                <label for="show_name">Xem tên thí sinh khi chấm điểm</label>
